declare namespace EcDiagnostics.Commons {
...
declare namespace EcDiagnostics.Commons {
    ...
    type WSSubscriptionCallbacks = {
        commands: ServerDef.Command[];
        interval: number;
    ...
    abstract class Communication {
        ...
        protected processReadValues<T = any>(callbacks: WSCallbacks, response: ServerDef.IMessage<T>): undefined | any;
        protected processReadValues_subscription<T = any>(callbacks: WSSubscriptionCallbacks, response: ServerDef.IMessage<T>): void;
        protected processReadValues_polling<T = any>(callbacks: WSCallbacks, response: ServerDef.IMessage<T>): void;
    }
}
declare namespace EcDiagnostics.Commons {
...
declare namespace EcDiagnostics.Commons {
    namespace Utility {
        ...
        function selectContainsOptionValue(select: HTMLSelectElement, value: string): boolean;
        function isDataComplex(val: any): boolean;
        function compareObjectsRecursion<T extends object>(oldObj: T, newObj: T, changes: Partial<T>): false | Partial<T>;
        function compareObjects<T extends object>(oldObj: T, newObj: T): Partial<T>;
        function baseConverter(num: string, baseFrom: number, baseTo: number): string;
        function fillStringToLength(character: string, numOfCharacters: number, stringToFillUp: string): string;
        function removeNotConnectedDomElements(domElements: Set<HTMLElement> | Map<HTMLElement, any>): void;
        function toHexString(dec: number): string;
    }
    namespace Vector {
        function getDirectionalVector(p1: XY, p2: XY): XY;
        function getVectorLength(p1: XY, p2?: XY): number;
        function getCenter(p1: XY, p2: XY): XY;
        function scalarProduct(scalar: number, p: XY): {
            x: number;
            y: number;
        };
        function sumVector(p1: XY, p2: XY): XY;
    }
}
declare namespace EcDiagnostics.Commons {
    namespace DrawUtility {
        function createCanvasLayers(parentEl: HTMLElement, numberOfLayers: number): DrawingLayer[];
        function drawGrid(ctx: CanvasRenderingContext2D, numberOfRows: number, numberOfCols: number, gridSpacing: number): void;
        function drawComponent(ctx: CanvasRenderingContext2D, colIndex: number, rowIndex: number, width: number, height: number, size: number): void;
    }
}
declare namespace EcDiagnostics.Commons {
    class Notifications {
...
declare namespace TcHmi.Controls.Beckhoff.TcHmiEcDiagnosticsClasses {
    class ControlCommunication extends EcDiagnostics.Commons.Communication {
        ...
        writeSymbol(symbol: string, writeValue: any, callback?: (data: any) => any, cbErrorEl?: {
            button?: EcDiagnostics.Commons.StatusButton;
            message?: HTMLElement;
        }): void;
    }
}
